...
<p>The <code>SetIntMSBPos()</code> method selects integer register byte order. The default (<code>false</code>) is most-significant byte first, which matches INA228/INA238. Pass <code>true</code> for least-significant byte first. This packing applies to integer helpers such as <code>ReadInt</code>, <code>ReadU24</code>, <code>ReadU40</code>, and <code>ReadRaw</code>. <code>ReadRegisterBytes</code> and <code>WriteRegisterBytes</code> copy bytes in bus order with no integer packing.</p>
<p>Whole-byte register widths from 1 through 8 bytes are supported: 8, 16, 24, 32, 40, 48, 56, and 64-bit convenience methods, plus <code>ReadRaw</code>/<code>WriteRaw</code> for an explicit byte count. Signed partial-width values (for example a signed 24-bit or 40-bit register) are sign-extended into the destination integer.</p>
